...
export const refreshAuth = () => (dispatch, getState) => {
  ...
  axios.get(API_REFRESH_URL, httpHeaderConfig(getState)).then(
    res => {
      dispatch({
        type: AUTH_LOADED,
        payload: res.data,
...
export const login = (email, password) => (dispatch, getState) => {
  ...
  axios.post(API_LOGIN_URL, formData, httpHeaderConfig(getState)).then(
    res => {
      dispatch({
        type: LOGIN_SUCCESS,
        payload: res.data,
...
export const logout = () => (dispatch, getState) => {
  dispatch({ type: AUTH_LOADING });
  axios.post(API_LOGOUT_URL, {}, httpHeaderConfig(getState)).then(
    res => {
      dispatch({
        type: LOGOUT_SUCCESS,
        payload: res.data,
...
export const forgotPassword = email => (dispatch, getState) => {
  const formData = new FormData();
  formData.append('email', email);
  axios.put(API_FORGOT_PASSWORD_URL, formData, httpHeaderConfig(getState)).then(
    res => {
      dispatch({
